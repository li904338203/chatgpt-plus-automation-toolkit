...
涓€寮犲崱鍙敤涓€娆★紝鐢ㄥ畬浠庢枃浠剁Щ闄ゃ€?
姣忓紶鍗¤嚜甯︽墜鏈哄彿鍜?API锛堝钩鍙版壒閲忓鍑烘牸寮忥級銆?
"""
...
# 缇庡浗 ZIP code 鍓?浣?鈫?宸炵缉鍐欐槧灏勶紙瑕嗙洊鎵€鏈?ZIP 鑼冨洿锛?
_ZIP3_TO_STATE = {
    ...
    "856": "AZ", "857": "AZ",
    "860": "NM", "861": "NM", "862": "NM", "863": "NM", "864": "NM",
    "865": "NM",
    "870": "NM", "871": "NM", "872": "NM", "873": "NM", "874": "NM",
    "875": "NM", "877": "NM", "878": "NM", "879": "NM",
    "880": "TX", "881": "TX", "882": "TX", "883": "TX", "884": "TX",
    "885": "TX",
...
def zip_to_state(zip_code: str) -> str:
    """Infer US state code from ZIP prefix."""
    prefix = zip_code[:3]
    return _ZIP3_TO_STATE.get(prefix, "")
...
def parse_card_line(line: str) -> CardInfo | None:
    """解析 cards.txt 单行记录，兼容 ---- 与 | 两种格式。"""
    ...
    if "----" in line:
        ...
        # 第一列可能是 KW-ID，跳过
        offset = 0
        if parts[0].startswith("KW-") or (not parts[0].isdigit() and len(parts[0]) > 16):
            offset = 1
        if len(parts) < offset + 7:
            return None

        number = re.sub(r"\s+", "", parts[offset])
        exp_raw = parts[offset + 1]
        cvv = parts[offset + 2]
        if not re.fullmatch(r"\d{13,19}", number):
            return None
        if not re.fullmatch(r"\d{3,4}", cvv):
            return None
        if not re.search(r"\d", exp_raw):
            return None
        phone = parts[offset + 3] if len(parts) > offset + 3 else ""
        holder = parts[offset + 4] if len(parts) > offset + 4 else ""
        address_raw = parts[offset + 5] if len(parts) > offset + 5 else ""
        sms_api = parts[offset + 6] if len(parts) > offset + 6 else ""

        # 解析有效期：2030/4 -> 04/30
        exp_parts = exp_raw.replace("-", "/").split("/")
        if len(exp_parts) == 2:
            a, b = exp_parts[0].strip(), exp_parts[1].strip()
            if len(a) == 4:
                exp_month = b.zfill(2)
                exp_year = a[2:]
            else:
        ...
        if address_raw:
            ...
            if len(addr_parts) >= 3:
                ...
                if m:
                    ...
                    state_guess = (m.group("state") or "").strip().upper()
                    zip_code = (m.group("zip") or "").strip()
                    if state_guess:
                        state_code = state_guess
                else:
                    tokens = city_zip.split()
                    if tokens and tokens[-1].isdigit():
                        zip_code = tokens[-1]
                        if len(tokens) >= 3 and len(tokens[-2]) == 2 and tokens[-2].isalpha():
                            state_code = tokens[-2].upper()
                            city = " ".join(tokens[:-2])
                        else:
                            city = " ".join(tokens[:-1])
                    else:
        ...
        if not state_code and zip_code and country == "US":
            state_code = zip_to_state(zip_code)

        return CardInfo(
            number=number, exp_month=exp_month, exp_year=exp_year, cvv=cvv,
            holder_name=holder, first_name=first_name, last_name=last_name,
            street=street, city=city, state=state_code, zip_code=zip_code,
            country=country, phone=phone, sms_api_url=sms_api, raw_line=line,
        )
    ...
    if "|" in line:
        ...
        if not re.search(r"\d", exp):
            return None
        exp_parts = exp.split("/")
        exp_month = exp_parts[0].zfill(2)
        exp_year = exp_parts[1] if len(exp_parts) > 1 else "30"
        phone = parts[3] if len(parts) > 3 else ""
        sms_api = parts[4] if len(parts) > 4 else ""
        holder = (parts[5] if len(parts) > 5 else "").rstrip(".")
        name_parts = holder.split()
        first_name = name_parts[0] if name_parts else "JOHN"
        last_name = " ".join(name_parts[1:]) if len(name_parts) > 1 else "DOE"
        street = parts[6] if len(parts) > 6 else ""
        city = parts[7] if len(parts) > 7 else ""
        state_code = parts[8] if len(parts) > 8 else ""
        zip_code = parts[9] if len(parts) > 9 else ""
        country = parts[10] if len(parts) > 10 else "US"
        return CardInfo(
            number=number, exp_month=exp_month, exp_year=exp_year, cvv=cvv,
            holder_name=holder, first_name=first_name, last_name=last_name,
            street=street, city=city, state=state_code, zip_code=zip_code,
...
class CardPool:
    def __init__(self, cards_file: str | Path = "data/paypal/cards.txt"):
        self.cards_file = resolve_path(cards_file)
    ...
    def count(self) -> int:
        if not self.cards_file.exists():
            return 0
        return sum(
            1 for line in self.cards_file.read_text(encoding="utf-8").splitlines()
            if parse_card_line(line) is not None
        )

    def take_one(self) -> CardInfo | None:
        """Get first available card without removing from file."""
        with self._lock:
            if not self.cards_file.exists():
                return None
            for line in self.cards_file.read_text(encoding="utf-8").splitlines():
                parsed = parse_card_line(line)
                if parsed:
                    return parsed
            return None

    def remove(self, card: CardInfo) -> None:
        """Remove used card from file after successful payment."""
        with self._lock:
            if not self.cards_file.exists():
                return
            lines = self.cards_file.read_text(encoding="utf-8").splitlines()
            remaining = [l for l in lines if l.strip() != card.raw_line.strip()]
            self.cards_file.write_text("\n".join(remaining) + ("\n" if remaining else ""), encoding="utf-8")

    def return_card(self, card: CardInfo) -> None:
        """Compatibility no-op: cards are no longer returned to pool."""
        pass
